...
# Save EMG signal data to a CSV file
def save_to_csv(time, amplitude, filename="emg_signal_data.csv"):
    try:
        ...
        print(f"EMG signal data saved to {filename}")
    except IOError as e:
...
if emg_signal:
    ...
    # Plot the original and filtered EMG signal
    plt.figure(figsize=(12, 6))
    plt.plot(time[5000:5500], emg_signal[5000:5500], label='Original EMG signal', color='red', alpha=0.6)
    plt.plot(time[5000:5500], filtered_emg_signal[5000:5500], label='Filtered EMG signal (50 Hz Removed)', color='blue', linewidth=1)
    plt.title("EMG signal from ESP32 (Before and After Filtering)")
    plt.xlabel("Time (s)")
